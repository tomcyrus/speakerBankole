...
export const Navbar: React.FC<NavbarProps> = ({ onNavigate, activePage }) => {
  ...
  return (
    <nav className="fixed w-full z-50 top-0">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
                ...
                <button
                    className="md:hidden text-zinc-900 p-2 focus:outline-none transition-transform active:scale-95"
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-white z-40 flex flex-col pt-24 px-6 md:hidden overflow-y-auto transition-all duration-300 ease-in-out ${
            isOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-full pointer-events-none'
        }`}
        style={{ height: '100dvh' }}
      >
          <div className="flex flex-col gap-6 text-xl font-medium text-zinc-900 w-full pb-10">
            {navLinks.map((link) => (
                <button
                key={link.value}
                onClick={() => {
                ...
                className={`py-3 text-left border-b border-zinc-50 transition-colors hover:text-amber-600 hover:pl-2 ${activePage === link.value ? 'text-amber-600 pl-2 border-amber-100' : ''}`}
                >
                {link.label}
                </button>
            ))}
